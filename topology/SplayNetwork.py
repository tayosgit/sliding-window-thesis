...
class SplayNetwork:
    ...
    #     """Druckt jeden Knoten des Baumes mit einer Einrückung, die seiner Tiefe im Baum entspricht."""
    #     if node is not None:
    #         # Drucke den rechten Teilbaum zuerst, mit erhöhter Einrückung
    #         self.print_tree(node.right, indent + 4)
    #         # Drucke den aktuellen Knoten: Einrückungen + Knotenwert
    #         print(' ' * indent + str(node.key))
    #         # Drucke den linken Teilbaum, mit erhöhter Einrückung
    ...
    #             indent += "|    "
    #         print(node.key)
    #         self.print_tree(node.left, indent, last=False)
    #         self.print_tree(node.right, indent, last=True)

    # def traverse_to_print(self, node):
    #     if node is not None:
    #         print(node)
    #         if node.left is not None:
    #             node = node.left
    #             self.traverse_to_print(node)
    ...
    def route(self, a, b):
        ...
        if parent_ca.key > common_ancestor.key or parent_ca.key < common_ancestor:
    ...
    def splay(self, node_a, node_b):
        ...
        if node_a == node_b:  # Case 1: Nodes are identical
            ...
            at_root = node_a == node_b.parent.parent
